...
fl_vel = np.multiply((1 / given_N), par_fl_vel)
fr_vel = np.multiply((1 / given_N), par_fr_vel)
rl_vel = np.multiply((1 / given_N), par_rl_vel)
rr_vel = np.multiply((1 / given_N), par_rr_vel)
...
for R in parameters[0]:
    for LW in parameters[1]:
        ...
        if bestError_rss == 0 or cumulativeError_rss < bestError_rss:
            ...
            bestParameter_rss = [R, LW]
            bar.text = "Best error: " + str(bestError_rss) + " -> parameters: " + str(bestParameter_rss)
