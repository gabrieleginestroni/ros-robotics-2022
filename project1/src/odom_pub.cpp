    ...
    public:
        ...
        void compOdometry(const geometry_msgs::TwistStamped::ConstPtr& msg){
            ...
            if (this->type == integrate_type::EULER) {
                x_new = this->x_old + (msg->twist.linear.x * cos(theta_old) - msg->twist.linear.y * sin(theta_old)) * time_s;
                y_new = this->y_old + (msg->twist.linear.x * sin(theta_old) + msg->twist.linear.y * cos(theta_old)) * time_s;
            } else {
                x_new = this->x_old +
